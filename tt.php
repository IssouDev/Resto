...
if (isset($_POST['firstname']) && isset($_POST['login']) ) {
  extract($_POST);

$sql="SELECT * from client WHERE nom='$firstname' and password='$password'" ;
$resault=mysqli_query($bdd,$sql);   

if ($row=mysqli_fetch_assoc($resault)) {
$_SESSION['firstname']= $row['nom'];
  # code...
}
...
<form method="POST" action="tt.php">
 ...
 height:90px; margin-top:0px ;"> <h1 style="color:rgb(51, 0, 51);">Developers restaurant</h1>
  <table class="ta"><tr><td style="color:white;">Already a client ?</td></tr>
  <tr><td><input type="text" name="firstname" placeholder="Firstname" required></td><td> <input type="password" name="password" placeholder="password" required</td><td><input  class="btn"  type="submit" name="login" value="login"></td></tr>
   <tr><td> <a href="signup.php" target="_blank" style="color:white;"> Singup here </a></td></tr>
  </table>
  </section>
  <br>
<center>
...
<div class="flo">
  <button class="btnn order w1 ">Order now !</button>                  
...
<table style="width:100% ; hight:180px; border:1px solid black;background-color: rgba(0, 153, 153,0.5);">
  <tr><td style="width:20%; color:rgb2(204,255,153); font-size:25px;">The Devolopers</td><td style="font-size:20px; color:rgb(51, 0, 51)">contact us</td> <td style="font-size:20px; color:rgb(51, 0, 51)"> Our restaurant</td><td style="font-size:20px; color:rgb(51, 0, 51)" >Our chefs</td></tr>
